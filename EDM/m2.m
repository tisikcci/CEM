...
Constant_part3 = -eta_/(4*pi);
%装填Z_MOM_1
Z_MOM_1 = zeros(Edg_MOM_Total,Edg_MOM_Total)+1j*zeros(Edg_MOM_Total,Edg_MOM_Total);
tic;
for m=1:Edg_MOM_Total
    %等效偶极矩方法准备
    dolp_MOM_r_m = repmat(dolp_MOM_r0(m,:),Edg_MOM_Total,1); %MOM区域第m个等效偶极矩中心位置
    dolp_MOM_r_m_n = dolp_MOM_r_m-dolp_MOM_r0;               %第m个偶极矩到其他所有偶极矩的位置关系
    norm_dolp_MOM_r_m_n = sqrt(sum(dolp_MOM_r_m_n.^2,2));    %所有偶极矩中心到第m个偶极矩的距离  n*1
    r = dolp_MOM_r_m_n./repmat(norm_dolp_MOM_r_m_n,1,3);     %所有电偶极矩到第m个电偶极矩的方向矢量
    CC = 1./norm_dolp_MOM_r_m_n.^2.*(1+1./(1j*k*norm_dolp_MOM_r_m_n)); %C
    jk_r = 1j*k./norm_dolp_MOM_r_m_n;                             %1jk/R     
    exp_r = exp(-1j*k*norm_dolp_MOM_r_m_n);                       %所有偶极矩到第m个偶极矩的相位关系%exp(-1jkR)
    ...
    mm = lm/2*(rho_m_P+rho_m_M);           %第m个电偶极矩
    %九点法准备
   ...
   for n=1:Edg_MOM_Total  
       if(norm_dolp_MOM_r_m_n(n,1)>deta)      %当第m个偶极矩相对与任意的偶极矩的距离大于0.15个波长
           Z_MOM_1(m,n)=Constant_part3*exp_r(n,:)*(mm*m_n(n,:)'*(jk_r(n,:)+CC(n,:))-...      %采用等效偶极矩方法进行填充自阻抗矩阵
                        (mm*r(n,:)')*(m_n(n,:)*r(n,:)')*(jk_r(n,:)+3*CC(n,:)));
       else
