...
for i=1:AA
    %做面元遮蔽判断：
    %自遮蔽判断
    afa0 = n_i*k_i(i,:)';              %所有入射方向矢量与面元法矢作用(每一列即是对应一个角度下时，所有面片的遮蔽判断角)
    sita = afa0<0;                %afa0<0的所有面元都置为1，afa0>=0的面元都被自身遮蔽，置为0
    %互遮蔽判断
    %此处预留再处理
    k_s_n = repmat(k_s(i,:),TrianglesTotal,1);
    h_i_n = repmat(h_i(i,:),TrianglesTotal,1);
...
%%
%根据散射场的值计算RCS
rcs_thta = 4*pi*r^2*(abs(sum(E.*e_r_thta_s,2).^2))/Z0^2;
rcs_phi = 4*pi*r^2*(abs(sum(E.*e_r_fire_s,2).^2))/Z0^2;
