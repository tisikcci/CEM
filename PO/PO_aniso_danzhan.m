...
Zuu = 1/sqrt(7)*Z0;%(0.1133+0.0166j)*Z0;        %主方向值设置
Zvv = 1/sqrt(7)*Z0;%(0.1133+0.0166j)*Z0;        %主方向值设置
...
e_i_phi = repmat([-sin(de_phi) cos(de_phi) 0],AA,1);                             %fire极化入射
...
E_inc = E_thta*e_i_theta+E_phi*e_i_phi; %总坐标系电场矢量极化表达
%%
%计算任意远场空间处的散射场
...
for i=1:AA
    ...
    %自遮蔽判断
    k_i_n = k_i(i,:);
    afa0 = n_i*k_i_n';              %所有入射方向矢量与面元法矢作用(每一列即是对应一个角度下时，所有面片的遮蔽判断角)
    ...
    %E_L_i = e_L_i*E_inc(i,:)'; %每个面片上场在其上的分量
    %E_I_i = e_T_i*E_inc(i,:)';
    %用来求解反射系数的参数
